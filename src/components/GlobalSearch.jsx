  ...
  return (
    <>
      <button
        ...
        className="w-9 h-9 flex items-center justify-center rounded-xl bg-surface-100 text-gray-500 active:scale-95 transition-all"
      >
        <Search size={18} />
      </button>
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-white">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-surface-200">
            ...
            <input
              ...
              className="flex-1 text-sm font-medium outline-none text-gray-900 placeholder:text-gray-400"
            />
            <button onClick={() => setOpen(false)} className="text-gray-400 active:scale-95">
              <X size={20} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
            {query && total === 0 && (
              <p className="text-sm text-gray-400 text-center py-12">Sin resultados para "{query}"</p>
            )}
                ...
                <div className="space-y-2">
                  {results.recipes.map(r => (
                    <button key={r.id} onClick={() => go(`/productos/${r.id}`)}
                      className="w-full flex items-center gap-3 p-3 bg-surface-50 rounded-xl text-left active:bg-surface-100">
            ...
            {results.ingredients.length > 0 && (
              <section>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Ingredientes</p>
                <div className="space-y-2">
                  {results.ingredients.map(i => (
                    <button key={i.id} onClick={() => go(`/stock/${i.id}`)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400">Stock: {i.stock} {i.unit}</p>
                      </div>
            ...
            {results.orders.length > 0 && (
              <section>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Comandas</p>
                <div className="space-y-2">
                  {results.orders.map(o => (
                    <button key={o.id} onClick={() => go(`/comandas/${o.id}`)}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-gray-900 truncate">{o.clientName || 'Sin nombre'}</p>
                        <p className="text-xs text-gray-400">{new Date(o.createdAt).toLocaleDateString('es-AR')}</p>
                      </div>
                      <p className="text-sm font-bold text-gold-600 shrink-0">
            ...
            {!query && (
              <p className="text-sm text-gray-400 text-center py-12">Escribí para buscar...</p>
            )}
          </div>
        </div>
      )}
