...
// Flujo: 0 = Bienvenida, 1 = País + Nombre
const STEPS = [
  {
    id: 'welcome',
...
export default function OnboardingPage() {
  ...
  const canProceed =
    step === 0 ? true :
    !!selectedCountry && businessName.trim().length >= 2
        ...
        {STEPS.map((_, i) => (
          <div
            ...
            className={`h-1.5 rounded-full transition-all duration-300 ${
              i <= step ? 'bg-primary-500 w-6' : 'bg-surface-200 w-2'
            }`}
          />
        ))}
      </div>
      ...
      <div className="flex-1 flex flex-col px-6 pt-8 pb-6 overflow-y-auto">
        ...
        {step === 1 && (
          <div className="flex flex-col gap-6">
            ...
            {/* País */}
            <div>
              <label className="label mb-2">¿De qué país sos?</label>
