    ...
    return (
      <div className="flex flex-col min-h-full bg-app">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8 text-center gap-5">
          ...
          <div>
            <h2 className="text-xl font-display font-bold text-app-primary mb-2">CRM de Clientes</h2>
          ...
          <div className="w-full space-y-2 text-left">
            {['📋 Historial completo de pedidos por cliente', '💰 Total gastado y deudas pendientes', '⭐ Producto favorito de cada cliente', '📱 Llamar o enviar WhatsApp directo'].map((f) => (
              <div key={f} className="flex items-center gap-2 text-sm text-app-muted">
                <span>{f}</span>
              </div>
            ))}
          </div>
        ...
        {clients.length > 0 && (
          <div className="px-4 pt-4">
            <div className="relative">
              <Search size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-app-faint" />
              <input
                type="text"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-bold text-app-primary truncate">{client.name}</p>
                        {s?.debt > 0 && (
                      ...
                      {client.phone && (
                        <p className="text-xs text-app-faint mt-0.5">{client.phone}</p>
                      )}
                    </div>
